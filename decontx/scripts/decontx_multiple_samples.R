...
for (sample in sampleDirs) {
  ...
  # Save marker percentage bar plots
  p4 <- plotDecontXMarkerPercentage(norm_sce, markers = markers, assayName = "counts")
  ggsave(filename = file.path(output_dir, paste0(sample_name, "_marker_percentage_counts.png")), plot = p4)
  
  p5 <- plotDecontXMarkerPercentage(norm_sce, markers = markers, assayName = "decontXcounts")
  ggsave(filename = file.path(output_dir, paste0(sample_name, "_marker_percentage_decontX.png")), plot = p5)
  
  p6 <- plotDecontXMarkerPercentage(norm_sce, markers = markers, assayName = c("counts", "decontXcounts"))
  ggsave(filename = file.path(output_dir, paste0(sample_name, "_marker_percentage_comparison.png")), plot = p6)
  
  # Save marker expression violin plots
  p7 <- plotDecontXMarkerExpression(norm_sce, markers = markers, ncol = 3)
  ggsave(filename = file.path(output_dir, paste0(sample_name, "_marker_expression_violin.png")), plot = p7)
